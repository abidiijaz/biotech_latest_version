...
class Screen1Controller extends Controller
{
    ...
    public function index(Request $request) {
        ...
        $pump_status = Pump_status::orderBy("id", "DESC")->first();
...
//        $res = json_encode($res);
    ...
    public function MassMonth(Request $req){
        if($req->isMethod('post')){
            $days=cal_days_in_month(CAL_GREGORIAN,$req->selectmonth,date("Y"));
            $starting = date("Y") . "-" .$req->selectmonth. "-" . "1";
            $ending = date("Y") . "-" .$req->selectmonth. "-" . $days;
            $val_1 = DB::select("SELECT * FROM monthly_mass_meth_bal WHERE date between '$starting' AND '$ending' ORDER BY mth_id ASC");
